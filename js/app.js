// ============================================================
// EGS — App Logic (cart, modal, catalog, filters)
// ============================================================
...
function addToCart(id, qty) {
  ...
  updateCartUI();
  toast(isAr() ? 'تمت الإضافة: ' + p.nameAr : 'Added: ' + p.name);
}
...
function updateCartUI() {
  ...
  const ar = isAr();
  if (n === 0) {
    area.innerHTML = `<div class="cart-empty"><div>🛒</div><p>${ar ? 'السلة فارغة' : 'Cart is empty'}</p></div>`;
  ...
  Object.values(cart).forEach(item => {
    const nm  = ar ? item.nameAr : item.name;
    const src = 'images/' + item.img + '.jpg';
    const ci  = document.createElement('div');
    ci.className = 'ci';
    ci.innerHTML = `
      <div class="ci-img"><img src="${src}" alt="${nm}"/></div>
      <div class="ci-info">
...
function sendCartWA() {
  const ar = isAr();
  let msg = ar ? 'مرحباً EGS! هذا طلبي:\n\n' : 'Hello EGS! My order:\n\n';
  Object.values(cart).forEach(item => {
    const nm = ar ? item.nameAr : item.name;
    msg += `• ${nm} (${item.model}) × ${item.qty}${item.price>0?' = $'+(item.price*item.qty).toFixed(2):' ('+( item.pd||'Contact for price')+')'}\n`;
  });
  msg += ar
    ? `\nالمجموع: $${cartTotal().toFixed(2)}\n\nأرجو الاستشارة حول التركيب.`
...
function renderCatalog() {
  ...
  if (!grid) return;
  const ar = isAr();
  const brandFilters = ['sonoff','tuya','summao'];

  let prods;
  if (currentFilter === 'all') {
    prods = PRODUCTS;
  } else if (brandFilters.includes(currentFilter)) {
    prods = PRODUCTS.filter(p => p.brand === currentFilter);
  } else {
    prods = PRODUCTS.filter(p => p.cat === currentFilter);
  }

  grid.innerHTML = '';
  prods.forEach(p => {
    const nm  = ar ? p.nameAr : p.name;
    const src = 'images/' + p.img + '.jpg';
    const card = document.createElement('div');
    card.className = 'prod-card';
    card.innerHTML = `
      <div class="prod-img-wrap">
        <div class="prod-img" onclick="openModal('${p.id}')">
          <img src="${src}" alt="${nm}" loading="lazy"/>
        </div>
        ${p.tag ? `<div class="prod-tag tag-${p.tc||'blue'}">${p.tag}</div>` : ''}
      </div>
      <div class="prod-body">
        <div class="prod-name" onclick="openModal('${p.id}')">${nm}</div>
        <div class="prod-model">${p.model}</div>
        <div class="prod-price">${p.pd}</div>
        <div class="prod-btns">
          <button class="prod-btn-cart" onclick="addToCart('${p.id}',1)">${ar?'أضف للسلة':'Add to Cart'}</button>
          <button class="prod-btn-detail" onclick="openModal('${p.id}')">${ar?'تفاصيل':'Details'}</button>
        </div>
      </div>`;
...
function openModal(id) {
  ...
  document.getElementById('mName').textContent  = ar ? p.nameAr : p.name;
  document.getElementById('mModel').textContent = p.model;
  document.getElementById('mPrice').textContent = getVariantPrice(p);
  document.getElementById('mDesc').textContent  = ar ? p.descAr : p.desc;
  ...
  if (p.variants && !Array.isArray(p.variants)) {
    const labels = ar
      ? {gangs:'عدد الأزرار',color:'اللون',protocol:'البروتوكول'}
      : {gangs:'Gang Count',color:'Color',protocol:'Protocol'};
    ...
    Object.keys(p.variants).forEach(key => {
      ...
      p.variants[key].forEach((val, i) => {
        (function(k, v) {
          ...
          if (k === 'color') {
            btn.style.cssText = `width:28px;height:28px;border-radius:50%;background:${colorMap[v]||'#ccc'};border:2px solid ${i===0?'var(--blue)':'#ccc'};padding:0;`;
            btn.title = v;
          } else {
            btn.textContent = v;
          }
...
function addFromModal() {
  ...
  const entry = {
    id:cartKey, brand:p.brand,
    name:p.name+(varStr?` (${varStr})`:''),
    nameAr:p.nameAr+(varStr?` (${varStr})`:''),
    model:p.model, img:getVariantImg(p),
    price:0, pd:getVariantPrice(p), qty
  };
  if (cart[cartKey]) { cart[cartKey].qty += qty; } else { cart[cartKey] = entry; }
  updateCartUI();
  toast(isAr() ? 'تمت الإضافة' : 'Added to cart');
  closeModal();
}
